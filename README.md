...
6. **Version** — Set the version in [`gradle.properties`](gradle.properties) (e.g. `4.0-Beta-17`). It must **not** end with `-SNAPSHOT` for a release on Maven Central. Pre-release labels like `4.0-Beta-17` are allowed; see [Version format](#version-format) and [Incrementing the version](#incrementing-the-version) below.
...
Artifacts appear on Maven Central after sync (usually within 10–30 minutes). Use the `version` from [`gradle.properties`](gradle.properties) (e.g. `4.0-Beta-17`).
...
Releases from **`4.0-Beta-17` onward** omit Gradle metadata so `mavenCentral()` alone is enough (see root `build.gradle`).
